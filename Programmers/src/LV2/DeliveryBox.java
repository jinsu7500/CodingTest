/**
 * * <알고리즘 스터디 32회차 2023.03.08>
 *  LV2_택배상자 : https://school.programmers.co.kr/learn/courses/30/lessons/131704
 * 문제 설명
...
public class DeliveryBox {
    public int solution2(int[] order) {
        ...
        for(int i = 0 ;i <order.length; i++){
            //컨베이너벨트 최상위 값이 order와 일치하면 적재  
            if((i+1) == order[answer]){
                answer++;
            }
            //일치하지 않고, 보조컨테이너에 적재되있을때
            else if (!subContainer.empty()){
                //보조컨테이너 최상위값과 현재 order값 비교
                if (order[answer] == subContainer.peek()){
                    answer++;
                    subContainer.pop();
                }
                //아닐경우 보조컨테이너에 추가적재
            ...
            //메인 레일이 없고, 보조컨테이너에 여러개가 쌓여있을수 있음.
            while(!subContainer.empty() && subContainer.peek() == order[answer]){                                   
                    answer++;
                    subContainer.pop();                
            }   
        }   
