...
public class Main
{
    public static void main(String args[]) throws IOException
    {
        ...
        for(int iCount = 0 ; iCount< num ; iCount++)
        {
            ...
            int xRow = Integer.parseInt(st.nextToken());
            int yCol = Integer.parseInt(st.nextToken());

            for(int ix = xRow ; ix < (xRow + 10); ix++)
            {
                for(int iy = yCol ; iy < (yCol + 10); iy++)
                {
                    iArray[ix][iy] = 1;
                }
            }
